import pandas as pd
import pytest

from humblepy.transform._pandas import _with_pandas_hash_value_column


def test_with_pandas_hash_value_column_sha256(numeric_with_nulls_pandas_df_fixture):
    """Tests the _with_pandas_hash_value_column() function using its default hash algorithm (SHA-256), concatenating strings with the default '||' and replacing nulls with the default '^^'.

    Args:
        numeric_with_nulls_pandas_df_fixture (callable): pytest fixture for a list of dicts containing mostly numeric test data, with some null (NoneType) values, to create a DataFrame with.

    """
    _df = numeric_with_nulls_pandas_df_fixture.copy()

    _df["hash_key"] = (
        "2aecd63a78bb594cc3a97782f95314fd09260958fe1dc61d17e57cbe4517afad",
        "d36106ae37598e73f4547235aedc203811d32b99970e711d8bd1d0dab0e9c496",
    )

    pd.testing.assert_frame_equal(
        _with_pandas_hash_value_column(
            numeric_with_nulls_pandas_df_fixture,
            columns_to_hash=["a", "b"],
            hash_algorithm="sha256",
            sort_columns=False,
            concat_with="||",
            replace_null_with="^^",
            uppercase=True,
            strip_whitespace=True,
            hash_value_column_name="hash_key",
        ),
        _df,
    )


def test_with_pandas_hash_value_column_sha512(numeric_pandas_df_fixture):
    """Tests the _with_pandas_hash_value_column() function using the SHA-512 hash algorithm, concatenating strings with the default '||' and replacing nulls with the default '^^'.

    Args:
        numeric_pandas_df_fixture (callable): pytest fixture for a list of dicts containing numeric test data, to create a DataFrame with.
    """

    _df = numeric_pandas_df_fixture.copy()

    _df["hash_key"] = (
        "2841a969f3d1a57b8316772a8e731a6571524c8124f388007d8af599b63fe2cbf7cb04896ece7f7fa5c5e607b1277bb93ead53d1a8ec60624c8f6eddcbb3f58f",
        "c080aeb6c24e62a6ee05c522deafc81f1abbf5338307ad1b350da889fc8a0ef2a964bf570f5ef769ecd132f21933dd24da8e714b484bbd65a01feec7a75386fc",
    )

    pd.testing.assert_frame_equal(
        _with_pandas_hash_value_column(
            numeric_pandas_df_fixture,
            columns_to_hash=["a", "b"],
            hash_algorithm="sha512",
            sort_columns=False,
            concat_with="||",
            replace_null_with="^^",
            uppercase=True,
            strip_whitespace=True,
            hash_value_column_name="hash_key",
        ),
        _df,
    )


def test_with_pandas_hash_value_column_md5(string_with_nulls_pandas_df_fixture):
    """Tests the _with_pandas_hash_value_column() function using the MD5 hash algorithm, concatenating strings with '-' and replacing nulls with '#'.

    Args:
        string_with_nulls_pandas_df_fixture (callable): pytest fixture for a list of dicts containing mostly string test data, with some null (NoneType) values, to create a DataFrame with.
    """

    _df = string_with_nulls_pandas_df_fixture.copy()

    _df["hash_key"] = (
        "930511fa326b6bca3b817ed81cfc2407",
        "a7a8cced4abd6e8f983a133f6ddc84a5",
    )

    pd.testing.assert_frame_equal(
        _with_pandas_hash_value_column(
            string_with_nulls_pandas_df_fixture,
            columns_to_hash=["a", "b"],
            hash_algorithm="md5",
            sort_columns=False,
            concat_with="-",
            replace_null_with="#",
            uppercase=False,
            strip_whitespace=True,
            hash_value_column_name="hash_key",
        ),
        _df,
    )
